...
- Probed 2026-06-18, read-only, against a single production tenant using
  ordinary API credentials. No `POST`/`PUT`/`DELETE` was sent.
...
| user_security_settings | ✅ (index→1) | n/a | Deeply nested RBAC matrix (roles, components, privileges) |
...
| **Device → person** | n/a | ❌ **impossible** (no field) |
| Test → Evidence | n/a | ❌ impossible (no list, no relationship, no link field) |
